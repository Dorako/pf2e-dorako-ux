class SettingsMenuDorakoUX extends FormApplication {
  static namespace;
  ...
  static get defaultOptions() {
    ...
    return mergeObject(options, {
      title: `pf2e-dorako-ux.settings.${this.namespace}.name`,
  ...
  static registerSettings() {
    ...
    for (const setting of this.SETTINGS) {
      game.settings.register("pf2e-dorako-ux", `${this.prefix}${setting}`, {
        ...settings[setting],
        config: true,
      });
    }
  ...
  async getData() {
    ...
    const templateData = Object.entries(settings).map(([key, setting]) => {
      const value = game.settings.get("pf2e-dorako-ux", `${this.prefix}${key}`);
      return {
        ...setting,
        key,
        value,
    ...
    return mergeObject(await super.getData(), {
      ...
      instructions: `pf2e-dorako-ux.settings.${this.namespace}.hint`,
    });
  }
  ...
  async _updateObject(_event, data) {
    for (const key of this.constructor.SETTINGS) {
      ...
      await game.settings.set("pf2e-dorako-ux", settingKey, data[key]);
    }
  }
}

export { SettingsMenuDorakoUX };
